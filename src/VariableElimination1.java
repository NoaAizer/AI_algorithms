...
public class VariableElimination1 {

	public static int i = 0;

	public static Factor1 inference(Factor1[] factorList,
									String[] queryVariables,
									String[] orderedListOfHiddenVariables,
									HashMap<String,String> evidenceList) {
		ArrayList<Factor1> restrictedFactorList = new ArrayList<Factor1>();
		// Restrict factors.
		for (int i = 0; i < factorList.length; i++) {
			Factor1 restrictedFactor = factorList[i];
			for (String variable : evidenceList.keySet()) {
				Integer position = restrictedFactor.getPositionByVariable(variable);
				if (position != null) {
		...
		for (String hiddenVar : orderedListOfHiddenVariables) {
			...
			ArrayList<Factor1> factorsToMultiply = new ArrayList<Factor1>();
			String factorMultiplyString = "";
			for (Factor1 restrictedFactor : restrictedFactorList) {
				Set<String> restrictedFactorVars = restrictedFactor.getVarsSet();
				if (restrictedFactorVars.contains(hiddenVar)) {
					...
					factorMultiplyString += restrictedFactor.factorTitle;
				}
			}
			...
			Factor1 multipliedFactorResult = factorsToMultiply.get(0);
			for (int i = 1; i < factorsToMultiply.size(); i++) {
				System.out.print(multipliedFactorResult.factorTitle + " * " + factorsToMultiply.get(i).factorTitle + " = ");
			...
			Factor1 summedOutFactor = sumout(multipliedFactorResult, hiddenVar);
			System.out.println("Summout " + hiddenVar + " from " + multipliedFactorResult.factorTitle);
			...
			for (Factor1 f : factorsToMultiply) {
				restrictedFactorList.remove(f);
			}

			// Print updated Factor list.
			System.out.print("Updated Factor Set: { ");
			for (Factor1 f : restrictedFactorList) {
				System.out.print(f.factorTitle + " ");
			}
			System.out.println("}\n");
		}
		...
		Factor1 multipliedFactorResult = restrictedFactorList.get(0);
		for (int i = 1; i < restrictedFactorList.size(); i++) {
			System.out.print(multipliedFactorResult.factorTitle + " * " + restrictedFactorList.get(i).factorTitle + " = ");
	...
	public static Factor1 normalize(Factor1 factor) {
		Factor1 returnFactor = new Factor1(factor.intToVariableNameMapping, i++);
		double sum = 0;
	...
	public static Factor1 sumout(Factor1 factor, String variable) {
		...
		Factor1 returnFactor = new Factor1(newVariables, i++);
	...
	public static Factor1 multiply(Factor1 factor1, Factor1 factor2) {
		Set<String> factor1Vars = factor1.getVarsSet();
		Set<String> factor2Vars = factor2.getVarsSet();
		...
		Factor1 returnFactor = new Factor1(unionArray, i++);
	...
	public static Factor1 restrict(Factor1 factor, String variable, String value) {
		...
		Factor1 newFactor = new Factor1(varSet, i++, variable, value);

		int position = factor.getPositionByVariable(variable);

		ArrayList<Row> rowList = factor.getRowList();
		for (Row row : rowList) {
			ArrayList<String> newTFList = new ArrayList<String>(row.ValuesList);
			newTFList.remove(position);
			if (row.ValuesList.get(position).equals(value)) {
